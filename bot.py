...
class GenerateKeyModal(discord.ui.Modal, title="Generate License Key"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Attempt to deliver key to user's Direct Messages (DM)
        dm_sent = False
        try:
        ...
        if dm_sent:
            await interaction.followup.send("✅ Your generated license key has been sent to your **Direct Messages (DMs)**!", ephemeral=True)
        else:
...
async def generate_command(
    ...
    if dm_sent:
        await interaction.followup.send("✅ Your generated key(s) have been sent to your **Direct Messages (DMs)**!", ephemeral=True)
    else:
